...
def generate_dilated_sliding_window_outside(
    window_size: int, dilation: int
) -> _mask_mod_signature:
    """Generates a mask that is dense within a window and dilated outside of it.

    Args:
        window_size: The size of the dense sliding window.
        dilation: The dilation factor for positions outside the window.
    """

    def dilated_sliding_window_outside(b, h, q_idx, kv_idx):
        diff = torch.abs(q_idx - kv_idx)
        in_window = diff <= window_size
        is_dilated = (diff % dilation) == 0
        return in_window | is_dilated

    dilated_sliding_window_outside.__name__ = (
        f"dilated_sliding_window_outside_{window_size}_dilation_{dilation}"
    )
    return dilated_sliding_window_outside


if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    B, H, SEQ_LEN, HEAD_DIM = 1, 1, 1024, 8
